...
class CLTrainer(Trainer):
    ...
    def evaluate(
        ...
        if arguana_eval==True:
            # return "no eval"
            eval_dataloader = self.get_eval_dataloader()
            # original_batch=[]
            # for batch in eval_dataloader:
                # original_batch.append(batch)
            self.model.eval()
            loss=[]
            with torch.no_grad():
                for batch in eval_dataloader:
                    ...
                    #     print(input_ids[:,:,:40])
                    # assert(are_identical_lines is False)

                    # for i in range(input_ids.size(0)):
                        # are_identical_lines=torch.unique(input_ids[i], dim=0).size(0)!=input_ids.size(1)
                        # if are_identical_lines is True:
                            # print(i,input_ids[i])
                            # assert(False)

                    # print(batch["input_ids"].shape,batch)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if step%(len(epoch_iterator)//3)==0 and step>0:
                    ...
                    eval_results = round(self.evaluate(arguana_eval=True).item(), 3)  # Assuming this returns a scalar
